...
/// Includes Level-specific Screen Warping (Tunnels) and Grid Snapping.
/// </summary>
...
public class testMove : MonoBehaviour
{
    ...
    public float powerUpDuration = 8f; // Matches GameManager/Upgrades naming
    ...
    void ApplyPurchasedUpgrades()
    {
        ...
        {
            Debug.LogWarning("!!! HUD/PLAYER: Cannot find PlayerUpgrades! Is it in the scene?");
        ...
        // Apply speed and power duration from upgrades
    ...
    private void Move()
    {
        ...
        {
            // Only snap to center if we aren't currently in the warp tunnel zone
            if (pos.x < (rightBoundary - 0.5f) && pos.x > (leftBoundary + 0.5f))
            {
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        switch (other.tag)
        {
            ...
            case "Fruit":
                other.GetComponent<FruitPickup>()?.Collect(this);
                break;
